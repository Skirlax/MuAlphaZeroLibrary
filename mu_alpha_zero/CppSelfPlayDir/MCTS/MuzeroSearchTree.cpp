...
MuzeroSearchTree::MuzeroSearchTree(py::object game_manager, std::map<std::string, py::object> config_args) {
    ...
    this->frame_buffer = std::make_unique<MuZeroFrameBuffer>(config_args["frame_buffer_size"].cast<int>(),this->game_manager.attr("get_noop")().cast<int>(),config_args["net_action_size"].cast<int>());
...
std::unique_ptr<std::vector<PlayeOneStepReturn>> MuzeroSearchTree::playOneGame(MuZeroDefaultNet* networkWrapper) {
    ...
    std::unique_ptr<std::vector<PlayeOneStepReturn>> data = std::make_unique<std::vector<PlayeOneStepReturn>>();
    for (int step = 0;step < numSteps; step++) {
    	...
    	std::tuple<torch::Tensor, torch::Tensor> pi_v = networkWrapper->predictionForward(std::get<1>(searchReturn.rootValLatent).unsqueeze(0).to(torch::kCUDA), true);
        ...
        string uuid_str = uuid.str();
        //uuid_str.replace(uuid_str.begin(), uuid_str.end(), { "-","_" });
        std::string framePath = this->config_args["pickle_dir"].cast<std::string>() + "/array_" + uuid_str + ".pth";
...
SearchReturn MuzeroSearchTree::search(MuZeroDefaultNet* networkWrapper, torch::Tensor state, double tau) {
    int currentBufferSize = this->frame_buffer->size();
    if (currentBufferSize == 0) {
        this->frame_buffer->initBuffer(std::move(state));
    }
    ...
    rootNode->expand(state_.to(torch::kCPU),pi,0.0);
    for (int simulation = 0; simulation < num_simulations; simulation++)
    {
        ...
        std::tuple<torch::Tensor, torch::Tensor> stateRew = networkWrapper->dynamicsForward(current_node_state_with_action.unsqueeze(0).to(c10::kCUDA),true);
        ...
        if (v.is_none() || !v.cast<bool>())
        {
            ...
            currentNode->expand(std::get<0>(stateRew).to(torch::kCPU),pi,reward);
        }
		else
		{
            value = v.cast<double>();
    ...
    std::map<int,double> action_probabilities = rootNode->get_self_action_probabilities(tau,true);
...
{
    double G = 0;
    double gamma = this->config_args["gamma"].cast<double>();
